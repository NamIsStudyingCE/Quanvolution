...
(1) \textbf{Data Regime Dependency:} On small-sample, imbalanced data (BreastMNIST), fixed quantum filters achieve superior ranking performance and variance stability: \texttt{Fixed Basic L2} attains the highest ROC-AUC of $\mathbf{0.8521 \pm 0.0095}$ vs. Classical CNN $0.8336 \pm 0.0259$, while \texttt{Fixed Strongly L2} achieves the highest PR-AUC of $\mathbf{0.9182 \pm 0.0071}$ with a $\sim 2.7\times$ smaller standard deviation. Conversely, on large multi-class data (OCTMNIST), the classical CNN baseline overwhelmingly dominates all quantum configurations (ROC-AUC $\mathbf{0.7505 \pm 0.0240}$, $d = +2.108, p < 0.001$), demonstrating that quantum expressibility is constrained by an architectural capacity bottleneck in multi-class regimes.
(2) \textbf{True Value of Trainability:} Optimizing parameter angles within the quantum kernel is beneficial only within the same ansatz family: on OCTMNIST, \texttt{Trainable Strongly} ($0.6922 \pm 0.0199$) significantly outperforms \texttt{Fixed Strongly} ($0.6690 \pm 0.0055$) with $\Delta = +0.0232$, $d = +1.050$, $p_{\text{wilcoxon}} = 0.0098$, yet only ties an appropriately chosen fixed random ansatz (\texttt{Fixed random\_L1} $0.6912 \pm 0.0071$, $p = 0.8875$, ns).
...
\midrule
Azevedo et al. (2022) \cite{azevedo2022quantum} & Breast (Mammography) & Hybrid VQC Head & Single split (No seeds) & Classical backbone dominates capacity; unable to isolate pure quantum kernel effect. \\
\midrule
K{\"u}bler et al. (2021) \cite{kubler2021inductive} / Schuld \& Killoran (2022) \cite{schuld2022advantage} & Kernel ML / Vision & Quantum Kernels / QNN & Theoretical bounds / Multi-dataset & Critical perspective showing advantage claims often evaporate against fair classical baselines. \\
\midrule
Matondo-Mvula and Elleithy (2024) \cite{matondo2024breast} & BreastMNIST (Ultrasound) & Quanv QNN ($9$-qubit, $3\times3$) & Single run (No seeds/tests) & Simulator-only; $28\times28$ downscaled to $14\times14$; no multi-seed significance testing. \\
\midrule
...
\bibitem{vu2024exploring} Vu, T.H., Le, H.L., Pham, T.B.: Exploring the features of quanvolutional neural networks for improved image classification. Quantum Machine Intelligence \textbf{6}(1), 29 (2024)
...
\bibitem{azevedo2022quantum} Azevedo, V., Silva, C., Dutra, I.: Quantum transfer learning for breast cancer detection. Quantum Machine Intelligence \textbf{4}(1), 5 (2022)
...
\bibitem{matondo2024breast} Matondo-Mvula, N., Elleithy, K.: Breast cancer detection with quanvolutional neural networks. Entropy \textbf{26}(8), 630 (2024)
\bibitem{schuld2022advantage} Schuld, M., Killoran, N.: Is quantum advantage the right goal for quantum machine learning? PRX Quantum \textbf{3}(3), 030101 (2022)
\end{thebibliography}
