...
Mọi cặp 'mean ± std' (cả ký hiệu ± và LaTeX \\pm) được tra theo mean trong
reconciliation_canonical.json. Thêm NOTE header đánh dấu báo cáo lịch sử."""
import json, re
...
# ký hiệu ± (U+00B1) và LaTeX \pm — dùng re.escape để tránh lỗi escaping
SYM = re.escape(chr(0x00B1)) + '|' + re.escape(chr(92) + 'pm')
PAIR = re.compile(r'(\d\.\d{4})\s*(?:' + SYM + r')\s*(\d\.\d{4})')
...
# verify: quét lại (cả hai ký hiệu)
for path in ['GD3/BAO_CAO_GIAI_DOAN_3.md', 'GD3/README.md']:
    t = open(path, encoding='utf-8').read()
    bad = []
    for m in PAIR.finditer(t):
        ...
        if mean in mean2std and abs(std - mean2std[mean]) > 5e-5:
            bad.append((mean, std))
    stale = [s for s in ['0.0309', '2.73', '[0.05, 0.25]', 'Sannakki', '0.0184', '0.0049',
                         '0.0093', '0.0490', '0.0448', '0.0145'] if s in t]
