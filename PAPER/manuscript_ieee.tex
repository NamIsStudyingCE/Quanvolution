...
\author{Hoang-Nam~Nguyen\textsuperscript{1} and Duy-Xuan-Bach~Nguyen\textsuperscript{1,*}\\
\textsuperscript{1}\textit{Faculty of Computer Engineering, University of Information Technology, Vietnam National University Ho Chi Minh City (VNU-HCM), Ho Chi Minh City, Vietnam}\\
\textit{Primary Author (Student):} \texttt{ng.h.nam0802@gmail.com} \quad \textit{*Academic Advisor \& Corresponding Author:} \texttt{bachndx@uit.edu.vn}%
\thanks{H.-N. Nguyen (First Author) is an undergraduate researcher with the Faculty of Computer Engineering, University of Information Technology, VNU-HCM, Ho Chi Minh City, Vietnam (e-mail: \texttt{ng.h.nam0802@gmail.com}).}%
\thanks{D.-X.-B. Nguyen (Academic Supervisor and Corresponding Author) is with the Faculty of Computer Engineering, University of Information Technology, VNU-HCM, Ho Chi Minh City, Vietnam (e-mail: \texttt{bachndx@uit.edu.vn}).}%
...
Despite surging interest, recent QML literature in biomedical imaging \cite{vu2024exploring, altares2025automatic, sannakki2026medical, hoang2023efficient} exhibits three fundamental methodological limitations:
\begin{itemize}
    \item \textbf{L1 --- Unfair Baseline Comparisons:} Prior studies frequently evaluate QML models against undertuned, arbitrary classical baselines or compare small quantum models against multi-million-parameter pretrained architectures (e.g., ResNet-18) without parameter isolation \cite{altares2025automatic}. Consequently, it remains impossible to determine whether reported performance stems from quantum transformations or classical classifier capacity.
    \item \textbf{L2 --- Absence of Multi-Seed Statistical Rigor:} A majority of published works report single-run experiments or 3-seed averages without reporting confidence intervals or non-parametric significance tests, conflating random initialization luck with genuine ``quantum advantage'' \cite{do_we_really_need_qml2026}.
...
\centering
\begin{tabular}{lp{3.2cm}p{3.2cm}p{2.6cm}cp{3.4cm}}
\toprule
...
\midrule
Henderson et al. (2019) \cite{henderson2020quanvolutional} & Synthetic MNIST & Random Quanvolution & Basic CNN & $1 - 3$ seeds (No tests) & Toy dataset, no medical scope, asymmetrical heads. \\
Cong et al. (2019) \cite{cong2019quantum} & Quantum Phase Detection & Pure QCNN & Classical MLP & Single-run theoretical & Tailored for quantum spin chains, incompatible with 2D grids. \\
Altares-López et al. (2025) \cite{altares2025automatic} & Industrial / Medical & HQCNN Hybrid & ResNet-18 (Pretrained) & Inconsistent seeds & Unfair baseline; unable to isolate quantum kernel gains. \\
Nature Sci. Rep. (2026) \cite{sannakki2026medical} & MedMNIST (PathMNIST) & VQC on IBM Quantum & Basic MLP & $3 - 5$ seeds (Untuned) & Weak baseline; device noise obscures algorithmic validity. \\
Critical Survey (2026) \cite{do_we_really_need_qml2026} & Critical QML Survey & Various QNNs & Modern CNNs & Literature Review & Identifies widespread over-claiming in QML literature. \\
\midrule
...
The end-to-end architecture consists of four sequential stages:
\begin{enumerate}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model Family} & \textbf{Kernel Params} & \textbf{Head ($K=2$)} & \textbf{Head ($K=4$)} & \textbf{Total Params} \\
\midrule
...
Table~\ref{tab:breastmnist} summarizes test performance across 10 independent seeds on BreastMNIST ($L=2$).

\begin{table*}[t]
\caption{10-seed empirical performance on BreastMNIST ($L=2$, 20 Epochs). Bold indicates best performance per column; $[ \cdot ]$ denotes $95\%$ CI.}
...
\midrule
\textbf{Classical CNN Baseline} & $0.8115 \pm 0.0281$ & $0.6875 \pm 0.0448$ $[0.6554, 0.7195]$ & $0.8799 \pm 0.0188$ & $0.4741 \pm 0.0830$ & $0.8336 \pm 0.0246$ $[0.8160, 0.8512]$ & $0.9041 \pm 0.0095$ $[0.8973, 0.9109]$ \\
\textbf{Fixed Basic Quanv (L2)} & $0.8192 \pm 0.0101$ & $0.6816 \pm 0.0259$ & $0.8874 \pm 0.0058$ & $0.4874 \pm 0.0378$ & $\mathbf{0.8521 \pm 0.0090}$ $[0.8457, 0.8586]$ & $0.9110 \pm 0.0065$ $[0.9063, 0.9156]$ \\
\textbf{Trainable Basic Quanv (L2)} & $0.8186 \pm 0.0224$ & $0.6732 \pm 0.0418$ & $0.8862 \pm 0.0145$ & $0.4814 \pm 0.0692$ & $0.8406 \pm 0.0239$ $[0.8235, 0.8577]$ & $0.9173 \pm 0.0184$ $[0.9041, 0.9304]$ \\
\textbf{Fixed Strongly Quanv (L2)} & $0.7994 \pm 0.0098$ & $0.6602 \pm 0.0213$ & $0.8733 \pm 0.0065$ & $0.4301 \pm 0.0348$ & $0.8139 \pm 0.0143$ $[0.8037, 0.8241]$ & $\mathbf{0.9182 \pm 0.0067}$ $[0.9134, 0.9230]$ \\
\textbf{Trainable Strongly Quanv (L2)} & $\mathbf{0.8205 \pm 0.0257}$ & $\mathbf{0.6945 \pm 0.0428}$ $[0.6639, 0.7251]$ & $\mathbf{0.8877 \pm 0.0163}$ & $\mathbf{0.4911 \pm 0.0756}$ & $0.8306 \pm 0.0280$ $[0.8106, 0.8506]$ & $0.9167 \pm 0.0087$ $[0.9105, 0.9229]$ \\
\bottomrule
\end{tabular}
\end{table*}

\begin{table*}[t]
\caption{10-seed empirical performance on OCTMNIST ($L=1$, 20 Epochs).}
\label{tab:octmnist}
\centering
\begin{tabular}{lcccccc}
\toprule
\textbf{Model Architecture} & \textbf{Accuracy} & \textbf{Balanced Acc} & \textbf{F1-Score} & \textbf{MCC} & \textbf{ROC-AUC} & \textbf{PR-AUC} \\
\midrule
\textbf{Classical CNN Baseline} & $\mathbf{0.5298 \pm 0.0239}$ & $\mathbf{0.4433 \pm 0.0199}$ $[0.4291, 0.4575]$ & $\mathbf{0.4792 \pm 0.0241}$ & $\mathbf{0.3092 \pm 0.0314}$ & $\mathbf{0.7505 \pm 0.0240}$ $[0.7333, 0.7676]$ & $\mathbf{0.4991 \pm 0.0268}$ $[0.4799, 0.5182]$ \\
\textbf{Fixed Basic Quanv (L1)} & $0.4726 \pm 0.0100$ & $0.4075 \pm 0.0077$ & $0.4300 \pm 0.0100$ & $0.2328 \pm 0.0130$ & $0.6711 \pm 0.0042$ $[0.6681, 0.6741]$ & $0.4186 \pm 0.0038$ $[0.4159, 0.4213]$ \\
\textbf{Trainable Basic Quanv (L1)} & $0.4650 \pm 0.0139$ & $0.3955 \pm 0.0115$ & $0.4179 \pm 0.0145$ & $0.2196 \pm 0.0191$ & $0.6704 \pm 0.0106$ $[0.6628, 0.6780]$ & $0.4102 \pm 0.0076$ $[0.4048, 0.4156]$ \\
\textbf{Fixed Champ GĐ2 (\texttt{random\_L1})} & $0.4799 \pm 0.0113$ & $0.4048 \pm 0.0099$ & $0.4269 \pm 0.0133$ & $0.2391 \pm 0.0165$ & $0.6912 \pm 0.0071$ $[0.6861, 0.6963]$ & $0.4443 \pm 0.0071$ $[0.4392, 0.4494]$ \\
\textbf{Fixed Strongly Quanv (L1)} & $0.4778 \pm 0.0087$ & $0.4034 \pm 0.0062$ & $0.4257 \pm 0.0114$ & $0.2348 \pm 0.0126$ & $0.6690 \pm 0.0055$ $[0.6651, 0.6690]$ & $0.4175 \pm 0.0049$ $[0.4140, 0.4210]$ \\
\textbf{Trainable Strongly Quanv (L1)} & $0.4772 \pm 0.0184$ & $0.4020 \pm 0.0161$ & $0.4246 \pm 0.0201$ & $0.2388 \pm 0.0259$ & $0.6922 \pm 0.0199$ $[0.6780, 0.7065]$ & $0.4365 \pm 0.0125$ $[0.4276, 0.4455]$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Fixed Basic Wins on ROC-AUC:} \texttt{Fixed Basic L2} achieves the highest ROC-AUC of $\mathbf{0.8521 \pm 0.0090}$, outperforming Classical CNN ($0.8336$) with statistical significance ($p_{\text{ttest}} = 0.0309, p_{\text{wilcoxon}} = 0.0298$) and a large effect size (\textbf{Cohen's $d = +0.815$}).
    \item \textbf{Fixed Strongly Wins on PR-AUC:} \texttt{Fixed Strongly L2} attains the highest PR-AUC of $\mathbf{0.9182 \pm 0.0067}$, significantly outperforming Classical CNN ($0.9041, p = 0.0023$) with a very large effect size (\textbf{Cohen's $d = +1.332$}).
    \item \textbf{Variance Stability:} Fixed quantum models exhibit standard deviations $\mathbf{2.73\times}$ smaller than Classical CNN in ROC-AUC ($0.0090$ vs. $0.0246$), confirming robustness to random seed variations.
...
Table~\ref{tab:octmnist} presents test results across 10 independent seeds on OCTMNIST ($L=1$).

\textbf{Key Statistical Findings on OCTMNIST:}
\begin{enumerate}
    \item \textbf{Conclusive Dominance of Classical CNN:} Classical CNN leads decisively across all 6 metrics (ROC-AUC $\mathbf{0.7505}$, PR-AUC $\mathbf{0.4991}$, Acc $\mathbf{0.5298}$), outperforming the best quantum model with $p < 0.001$ and huge effect sizes (\textbf{Cohen's $d = +2.108$} for ROC-AUC; \textbf{$d = +1.874$} for BAcc).
    ...
    \item \textbf{Statistical Tie with Fixed Champion:} \texttt{Trainable Strongly} ($0.6922$) only ties statistically with \texttt{Fixed random\_L1} ($0.6912, \Delta = +0.0010, p = 0.8875$, ns; Cohen's $d = +0.047$).
\end{enumerate}

\subsection{Optimization Dynamics \& Gradient Health}
Analyses of training trajectories confirm:
\begin{itemize}
...
\textbf{Fixed Quanv} & FE ($196$ patches) & $220.187\text{ ms}$ & $710.3\times$ & \textbf{$0$} \\
& Classifier Head & $0.034\text{ ms}$ & $0.11\times$ & $100\%$ Identical \\
...
\bibitem{altares2025automatic} F.~M.~Altares-López, A.~Ribeiro, and J.~J.~García-Ripoll, ``Automatic design of hybrid quantum-classical convolutional neural networks,'' \textit{Quantum Science and Technology}, vol.~10, no.~1, p.~015012, 2025.
\bibitem{sannakki2026medical} S.~S.~Sannakki et al., ``Medical image classification using quantum machine learning on NISQ devices,'' \textit{Scientific Reports}, vol.~16, p.~4512, 2026.
...
\bibitem{do_we_really_need_qml2026} A.~S.~C. et al., ``Do we really need quantum machine learning for computer vision? A critical empirical analysis,'' \textit{IEEE Trans. Pattern Anal. Mach. Intell.}, 2026.
