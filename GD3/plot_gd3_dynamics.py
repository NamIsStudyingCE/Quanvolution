...
def plot_gd3_training_curves(results, dataset_name, save_path='results/figures/gd3_curves.png'):
    ...
    for idx, m in enumerate(models):
        ...
        ax1.plot(epochs, np.mean(train_losses, axis=0), linestyle='--', color=col, alpha=0.4)
    ...
    for idx, m in enumerate(models):
        ...
        ax2.fill_between(epochs, mean_auc - std_auc, mean_auc + std_auc, color=col, alpha=0.1)
    ...
    plt.suptitle(f'3-Tier Quantum vs Classical Benchmark Dynamics: {dataset_name}', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def plot_gd3_theta_trajectories(results, target_model='trainable_strongly', save_path='results/figures/gd3_theta_trajectories.png'):
    ...
    for s_idx in range(min(len(hists), 3)):
        ...
        for p_idx in range(min(thetas.shape[1], 12)):
            ...
            ax.plot(epochs, thetas[:, p_idx], label=f'$\\theta_{{{p_idx+1}}}$', color=col, linewidth=1.6)

        ax.set_title(f'Seed {s_idx+1} ({target_model})', fontsize=11, fontweight='bold')
        ax.set_xlabel('Epoch', fontsize=10)
        ax.set_ylabel('Angle $\\theta$ (rad)', fontsize=10)
        ax.grid(True, linestyle=':', alpha=0.6)
        ax.legend(loc='best', fontsize=7, ncol=2)

    plt.suptitle(f'Quantum Angles ($\\theta$) Trajectories ({target_model})', fontsize=13, fontweight='bold', y=1.03)
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
...
def plot_gd3_gradient_norms(results, target_model='trainable_strongly', save_path='results/figures/gd3_gradient_norms.png'):
    ...
    ax.set_title(f'Quantum Gradient Norm Dynamics ({target_model})', fontsize=12, fontweight='bold')
